...
public class UploadAction extends BaseAction {
	...
	public String upload() {
		...
		if (!PreValidator.isNull(getUpload())) {
			try {
				FileUtils.copyFile(getUpload(), new File(path + File.separator + getUploadFileName()));
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
		...
		if (!PreValidator.isNull(getUploadFileName())) {
			ActionContext.getContext().put("message", "file upload successfully：" + getUploadFileName());
		}
